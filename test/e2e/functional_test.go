...
func (s *FunctionalSuite) TestFastFailOnPodTermination() {
	...
	// pods is forbidden: User "system:serviceaccount:work:default" cannot list resource "pods" in API group "" in the namespace "work"
	s.T().SkipNow()
	s.Given().
	...
	s.Given().
		...
		ExpectAuditEvents(func(t *testing.T, events *apiv1.EventList) {
			...
			for _, e := range events.Items {
				...
				isFailureEvent := e.Reason == work.EventReasonWorkflowFailed
	...
	s.Given().
		...
		ExpectAuditEvents(func(t *testing.T, events *apiv1.EventList) {
			...
			for _, e := range events.Items {
				...
				isSuccessEvent := e.Reason == work.EventReasonWorkflowSucceded
