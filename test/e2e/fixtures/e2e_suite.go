...
import (
	...
	"github.com/kubework/work/cmd/work/commands"
	...
	"github.com/kubework/work/util/kubeconfig"
	"github.com/kubework/work/workflow/packer"
)

const Namespace = "work"
const Label = "work-e2e"
...
func (s *E2ESuite) SetupSuite() {
	...
	s.wfClient = versioned.NewForConfigOrDie(s.RestConfig).KubeworkV1alpha1().Workflows(Namespace)
	s.wfTemplateClient = versioned.NewForConfigOrDie(s.RestConfig).KubeworkV1alpha1().WorkflowTemplates(Namespace)
	s.cronClient = versioned.NewForConfigOrDie(s.RestConfig).KubeworkV1alpha1().CronWorkflows(Namespace)
...
func (s *E2ESuite) DeleteResources(label string) {
	...
	// (as set in the  "workflows.kubework.io/workflow" label) does not exist.
	...
	// delete workflow pods
	{
		...
		// it seems "work delete" can leave pods behind
		pods, err := podInterface.List(metav1.ListOptions{LabelSelector: "workflows.kubework.io/workflow"})
		if err != nil {
			panic(err)
		}
		for _, pod := range pods.Items {
			workflow := pod.GetLabels()["workflows.kubework.io/workflow"]
...
func (s *E2ESuite) GetServiceAccountToken() (string, error) {
	...
	secretList, err := clientset.CoreV1().Secrets("work").List(metav1.ListOptions{})
	if err != nil {
		return "", err
	}
	for _, sec := range secretList.Items {
		if strings.HasPrefix(sec.Name, "work-server-token") {
