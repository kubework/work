import {Duration, Tabs, Ticker} from 'work-ui';
...
export const WorkflowNodeSummary = (props: Props) => {
    ...
    return (
        <div className='white-box'>
            ...
            <div>
                <button className='work-button work-button--base-o' onClick={() => props.onShowYaml && props.onShowYaml(props.node.id)}>
                    YAML
                </button>{' '}
                {template && (template.container || template.script) && (
                    <button className='work-button work-button--base-o' onClick={() => props.onShowContainerLogs && props.onShowContainerLogs(props.node.id, 'main')}>
                        LOGS
                    </button>
                )}
            </div>
...
export const WorkflowNodeContainer = (props: {
    ...
    return (
        <div className='white-box'>
            <div className='white-box__details'>{<AttributeRows attributes={attributes} />}</div>
            <div>
                <button className='work-button work-button--base-o' onClick={() => props.onShowContainerLogs && props.onShowContainerLogs(props.nodeId, container.name)}>
                    LOGS
                </button>
            </div>
        </div>
                ...
                <div className='row' key={artifact.name}>
                    <div className='columns small-1'>
                        <a href={artifact.downloadUrl}>
                            {' '}
                            <i className='icon work-icon-artifact' />
                        </a>
                    </div>
