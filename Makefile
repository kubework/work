...
ARGO_TOKEN            = $(shell kubectl -n work get secret -o name | grep work-server | xargs kubectl -n work get -o jsonpath='{.data.token}' | base64 --decode)

override LDFLAGS += \
  -X github.com/kubework/work.version=$(VERSION) \
  -X github.com/kubework/work.buildDate=${BUILD_DATE} \
  -X github.com/kubework/work.gitCommit=${GIT_COMMIT} \
  -X github.com/kubework/work.gitTreeState=${GIT_TREE_STATE}
...
ifneq ($(GIT_TAG),)
override LDFLAGS += -X github.com/kubework/work.gitTag=${GIT_TAG}
endif

ARGOEXEC_PKGS    := $(shell echo cmd/workexec            && go list -f '{{ join .Deps "\n" }}' ./cmd/workexec/            | grep 'kubework/work' | cut -c 26-)
CLI_PKGS         := $(shell echo cmd/work                && go list -f '{{ join .Deps "\n" }}' ./cmd/work/                | grep 'kubework/work' | cut -c 26-)
CONTROLLER_PKGS  := $(shell echo cmd/workflow-controller && go list -f '{{ join .Deps "\n" }}' ./cmd/workflow-controller/ | grep 'kubework/work' | cut -c 26-)
...
dist/work: vendor server/static/files.go $(CLI_PKGS)
	go build -v -i -ldflags '${LDFLAGS}' -o dist/work ./cmd/work

dist/work-linux-amd64: vendor server/static/files.go $(CLI_PKGS)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -v -i -ldflags '${LDFLAGS}' -o dist/work-linux-amd64 ./cmd/work

dist/work-linux-ppc64le: vendor server/static/files.go $(CLI_PKGS)
	CGO_ENABLED=0 GOOS=linux GOARCH=ppc64le go build -v -i -ldflags '${LDFLAGS}' -o dist/work-linux-ppc64le ./cmd/work

dist/work-linux-s390x: vendor server/static/files.go $(CLI_PKGS)
	CGO_ENABLED=0 GOOS=linux GOARCH=ppc64le go build -v -i -ldflags '${LDFLAGS}' -o dist/work-linux-s390x ./cmd/work

dist/work-darwin-amd64: vendor server/static/files.go $(CLI_PKGS)
	CGO_ENABLED=0 GOOS=darwin go build -v -i -ldflags '${LDFLAGS}' -o dist/work-darwin-amd64 ./cmd/work

dist/work-windows-amd64: vendor server/static/files.go $(CLI_PKGS)
	CGO_ENABLED=0 GOARCH=amd64 GOOS=windows go build -v -i -ldflags '${LDFLAGS}' -o dist/work-windows-amd64 ./cmd/work

.PHONY: cli-image
cli-image: dist/cli-image

dist/cli-image: dist/work-linux-amd64
	# Create CLI image
ifeq ($(DEV_IMAGE),true)
	cp dist/work-linux-amd64 work
	docker build -t $(IMAGE_NAMESPACE)/workcli:$(VERSION) --target workcli -f Dockerfile.dev .
	rm -f work
else
	docker build --no-cache -t $(IMAGE_NAMESPACE)/workcli:$(VERSION) --target workcli .
endif
	touch dist/cli-image
ifeq ($(K3D),true)
	k3d import-images $(IMAGE_NAMESPACE)/workcli:$(VERSION)
endif

.PHONY: clis
clis: dist/work-linux-amd64 dist/work-linux-ppc64le dist/work-linux-s390x dist/work-darwin-amd64 dist/work-windows-amd64 cli-image
...
dist/workexec-linux-amd64: vendor $(ARGOEXEC_PKGS)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -v -i -ldflags '${LDFLAGS}' -o dist/workexec-linux-amd64 ./cmd/workexec

.PHONY: executor-image
executor-image: dist/executor-image

dist/executor-image: dist/workexec-linux-amd64
	# Create executor image
ifeq ($(DEV_IMAGE),true)
	cp dist/workexec-linux-amd64 workexec
	docker build -t $(IMAGE_NAMESPACE)/workexec:$(VERSION) --target workexec -f Dockerfile.dev .
	rm -f workexec
else
	docker build --no-cache -t $(IMAGE_NAMESPACE)/workexec:$(VERSION) --target workexec .
endif
	touch dist/executor-image
ifeq ($(K3D),true)
	k3d import-images $(IMAGE_NAMESPACE)/workexec:$(VERSION)
endif
...
test/e2e/manifests/no-db/overlays/work-server-deployment.yaml: test/e2e/manifests/postgres/overlays/work-server-deployment.yaml
test/e2e/manifests/no-db/overlays/work-server-deployment.yaml:
	cat test/e2e/manifests/postgres/overlays/work-server-deployment.yaml | ./hack/auto-gen-msg.sh > test/e2e/manifests/no-db/overlays/work-server-deployment.yaml
...
test/e2e/manifests/no-db.yaml: $(MANIFESTS) $(E2E_MANIFESTS) test/e2e/manifests/no-db/overlays/work-server-deployment.yaml test/e2e/manifests/no-db/overlays/workflow-controller-deployment.yaml
...
test/e2e/manifests/mysql/overlays/work-server-deployment.yaml: test/e2e/manifests/postgres/overlays/work-server-deployment.yaml
test/e2e/manifests/mysql/overlays/work-server-deployment.yaml:
	cat test/e2e/manifests/postgres/overlays/work-server-deployment.yaml | ./hack/auto-gen-msg.sh > test/e2e/manifests/mysql/overlays/work-server-deployment.yaml
...
test/e2e/manifests/mysql.yaml: $(MANIFESTS) $(E2E_MANIFESTS) test/e2e/manifests/mysql/overlays/work-server-deployment.yaml test/e2e/manifests/mysql/overlays/workflow-controller-deployment.yaml
...
install: dist/postgres.yaml dist/mysql.yaml dist/no-db.yaml
	...
	kubectl get ns work || kubectl create ns work
ifeq ($(DB),postgres)
	kubectl -n work apply -f dist/postgres.yaml
else
ifeq ($(DB),mysql)
	kubectl -n work apply -f dist/mysql.yaml
else
	kubectl -n work apply -f dist/no-db.yaml
endif
endif
...
down:
	# Scale down
	kubectl -n work scale deployment/work-server --replicas 0
	kubectl -n work scale deployment/workflow-controller --replicas 0
	# Wait for pods to go away, so we don't wait for them to be ready later.
	[ "`kubectl -n work get pod -l app=work-server -o name`" = "" ] || kubectl -n work wait --for=delete pod -l app=work-server  --timeout 30s
	[ "`kubectl -n work get pod -l app=workflow-controller -o name`" = "" ] || kubectl -n work wait --for=delete pod -l app=workflow-controller  --timeout 2m

.PHONY: up
up:
	# Scale up
	kubectl -n work scale deployment/workflow-controller --replicas 1
	kubectl -n work scale deployment/work-server --replicas 1
	# Wait for pods to be ready
	kubectl -n work wait --for=condition=Ready pod --all -l app --timeout 2m
	# Token
	make env
...
logs:
	# Tail logs
	kubectl -n work logs -f -l app --max-log-requests 10 --tail 100
...
.PHONY: mysql-cli
mysql-cli:
	kubectl exec -ti `kubectl get pod -l app=mysql -o name|cut -c 5-` -- mysql -u mysql -ppassword work
...
test-api: test-images
	# Run API tests
	go test -timeout 3m -v -count 1 -p 1 -run WorkServerSuite ./test/e2e
...
publish-release: build
	...
	docker push $(IMAGE_NAMESPACE)/workcli:$(VERSION)
	docker push $(IMAGE_NAMESPACE)/workexec:$(VERSION)
