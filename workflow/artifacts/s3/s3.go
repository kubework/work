...
import (
	...
	"github.com/kubework/pkg/file"
	works3 "github.com/kubework/pkg/s3"
...
func (s3Driver *S3ArtifactDriver) newS3Client() (works3.S3Client, error) {
	opts := works3.S3ClientOpts{
		...
		Trace:     os.Getenv(common.EnvVarWorkTrace) == "1",
	}
	return works3.NewS3Client(opts)
}
...
func (s3Driver *S3ArtifactDriver) Load(inputArtifact *wfv1.Artifact, path string) error {
	err := wait.ExponentialBackoff(wait.Backoff{Duration: time.Second * 2, Factor: 2.0, Steps: 5, Jitter: 0.1},
		func() (bool, error) {
			...
			if !works3.IsS3ErrCode(origErr, "NoSuchKey") {
