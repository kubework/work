...
import (
	...
	"github.com/kubework/work/test/util"
...
func newWoc(wfs ...wfv1.Workflow) *wfOperationCtx {
	...
	_, err := fakeController.wfclientset.KubeworkV1alpha1().Workflows(wf.ObjectMeta.Namespace).Create(wf)
	if err != nil {
		panic(err)
	}
...
inputs:
  ...
  - name: manifest
    ...
    optional: true
    http:
        url: https://raw.githubusercontent.com/kubework/work/stable/manifests/install.yaml
script:
  image: alpine:latest
  command: [sh]
  source: |
