...
import (
	...
	workkubeerr "github.com/kubework/pkg/kube/errors"
	"github.com/kubework/pkg/strftime"
	...
	"github.com/kubework/work/pkg/apis/workflow"
	wfv1 "github.com/kubework/work/pkg/apis/workflow/v1alpha1"
	"github.com/kubework/work/pkg/client/clientset/versioned/typed/workflow/v1alpha1"
	"github.com/kubework/work/util/retry"
	"github.com/kubework/work/util/work"
	"github.com/kubework/work/workflow/common"
	"github.com/kubework/work/workflow/config"
	"github.com/kubework/work/workflow/packer"
...
type wfOperationCtx struct {
	...
	// auditLogger is the work audit logger
	auditLogger *work.AuditLogger
}
...
func newWorkflowOperationCtx(wf *wfv1.Workflow, wfc *WorkflowController) *wfOperationCtx {
	...
	woc := wfOperationCtx{
		...
		auditLogger:        work.NewAuditLogger(wf.ObjectMeta.Namespace, wfc.kubeclientset, wf.ObjectMeta.Name),
	}
...
func (woc *wfOperationCtx) operate() {
	...
	if woc.wf.Status.Phase == "" {
		...
		woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeNormal, Reason: work.EventReasonWorkflowRunning}, "Workflow Running")
		validateOpts := validate.ValidateOpts{ContainerRuntimeExecutor: woc.controller.GetContainerRuntimeExecutor()}
		wftmplGetter := templateresolution.WrapWorkflowTemplateInterface(woc.controller.wfclientset.KubeworkV1alpha1().WorkflowTemplates(woc.wf.Namespace))
		...
		if err != nil {
			...
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowFailed}, msg)
			return
		}
		...
		if err != nil {
			...
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowTimedOut}, "Workflow timed out")
	...
	if err != nil {
		...
		case ErrDeadlineExceeded:
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowTimedOut}, msg)
		default:
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowFailed}, msg)
		}
	...
	case wfv1.NodeSucceeded, wfv1.NodeSkipped:
		if onExitNode != nil && !onExitNode.Successful() {
			...
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowFailed}, onExitNode.Message)
		} else {
			woc.markWorkflowSuccess()
			woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeNormal, Reason: work.EventReasonWorkflowSucceded}, "Workflow completed")
		}
	case wfv1.NodeFailed:
		woc.markWorkflowFailed(workflowMessage)
		woc.auditLogger.LogWorkflowEvent(woc.wf, work.EventInfo{Type: apiv1.EventTypeWarning, Reason: work.EventReasonWorkflowFailed}, workflowMessage)
	case wfv1.NodeError:
...
func (woc *wfOperationCtx) persistUpdates() {
	...
	wfClient := woc.controller.wfclientset.KubeworkV1alpha1().Workflows(woc.wf.ObjectMeta.Namespace)
	...
	if err != nil {
		...
		if workkubeerr.IsRequestEntityTooLargeErr(err) {
...
// See https://github.com/kubework/work/issues/913
	...
	for _, ctr := range pod.Status.ContainerStatuses {
		...
		if ctr.Name != common.MainContainerName {
			if ctr.State.Terminated.ExitCode == 137 || ctr.State.Terminated.ExitCode == 143 {
				// if the sidecar was SIGKILL'd (exit code 137) assume it was because workexec
...
func (woc *wfOperationCtx) executeResource(nodeName string, templateScope string, tmpl *wfv1.Template, orgTmpl wfv1.TemplateHolder, boundaryID string) (*wfv1.NodeStatus, error) {
	...
	mainCtr.Command = []string{"workexec", "resource", tmpl.Resource.Action}
