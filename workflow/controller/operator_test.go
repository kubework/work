...
func TestOperateWorkflowPanicRecover(t *testing.T) {
	...
	_, err := controller.wfclientset.KubeworkV1alpha1().Workflows("").Create(wf)
...
func TestSidecarWithVolume(t *testing.T) {
	controller := newController()
	wfcset := controller.wfclientset.KubeworkV1alpha1().Workflows("")
...
func TestParamSubstitutionWithArtifact(t *testing.T) {
	...
	woc := newWoc(*wf)
	woc.operate()
	wf, err := woc.controller.wfclientset.KubeworkV1alpha1().Workflows("").Get(wf.ObjectMeta.Name, metav1.GetOptions{})
	assert.NoError(t, err)
...
metadata:
  ...
  labels:
    image: foo:bar
  annotations:
    k8s-webhook-handler.io/repo: "git@github.com:kubework/work.git"
...
func TestMetadataPassing(t *testing.T) {
	...
	for _, ev := range container.Env {
		switch ev.Name {
		case "REPO":
			assert.Equal(t, "git@github.com:kubework/work.git", ev.Value)
			foundRepo = true
		case "REVISION":
...
func TestResolvePlaceholdersInOutputValues(t *testing.T) {
	...
	templateString := pods.Items[0].ObjectMeta.Annotations["workflows.kubework.io/template"]
