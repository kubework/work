...
import (
	...
	workJson "github.com/kubework/pkg/json"
...
func NewSubmitCommand() *cobra.Command {
  ...
  work submit my-wf.yaml

# Submit a single workflow from an existing resource

  work submit --from cronwf/my-cron-wf
`,
...
func unmarshalWorkflows(wfBytes []byte, strict bool) []wfv1.Workflow {
	...
	var jsonOpts []workJson.JSONOpt
	if strict {
		jsonOpts = append(jsonOpts, workJson.DisallowUnknownFields)
	}
	err := workJson.Unmarshal(wfBytes, &wf, jsonOpts...)
	if err == nil {
