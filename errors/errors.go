...
// WorkError is an error interface that additionally adds support for
...
// workerr is the internal implementation of an Work error which wraps the error from pkg/errors
type workerr struct {
	code    string
	message string
	stracer stackTracer
...
func New(code string, message string) error {
	...
	return workerr{code, message, err.(stackTracer)}
}
...
func Cause(err error) error {
	if workErr, ok := err.(workerr); ok {
		return errors.Cause(workErr.stracer)
	}
	return errors.Cause(err)
}

func (e workerr) Error() string {
	return e.message
}

func (e workerr) Code() string {
	return e.code
}

func (e workerr) Message() string {
	return e.message
}

func (e workerr) StackTrace() errors.StackTrace {
	return e.stracer.StackTrace()
}

func (e workerr) JSON() []byte {
	type errBean struct {
...
func (e workerr) Format(s fmt.State, verb rune) {
	switch verb {
	case 'v':
		if s.Flag('+') {
